...
void scan_obj_files(struct GUI_directory_data* state) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        snprintf(full_path, sizeof(full_path), "%s/%s", state->obj_directory, entry->d_name);

        // Проверяем, является ли файл обычным
        if (stat(full_path, &file_stat) == 0 && S_ISREG(file_stat.st_mode)) {
            const char* ext = strrchr(entry->d_name, '.');
            if (ext && strcmp(ext, ".obj") == 0) {
                strncpy(state->obj_files[state->obj_file_count], entry->d_name, MAX_FILENAME_LENGTH - 1);
                state->obj_files[state->obj_file_count][MAX_FILENAME_LENGTH - 1] = '\0';
                state->obj_file_count++;

                if (state->obj_file_count >= MAX_FILES) break;  // Лимит на количество файлов
            }
        }
    }

    closedir(dir);
    printf("Found %d OBJ files in %s\n", state->obj_file_count, state->obj_directory);
