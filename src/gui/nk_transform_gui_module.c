void GUI_draw_transform_panel(struct nk_context *ctx, struct GUI_transform_panel *unit) {
  ...
  if (nk_begin(ctx, "Main", nk_rect(0, GUI_TRANSFORM_PANEL_Y_START, GUI_WIN_WIDTH_X, GUI_TRANSFORM_PANEL_Y_END), NK_WINDOW_BORDER | NK_WINDOW_BACKGROUND | NK_WINDOW_NO_SCROLLBAR )) {
    ...
    float ratio_two[] = {0.5f, 0.5f};
      ...
      nk_layout_row_dynamic(ctx, GUI_TRANSFORM_PANEL_POSITION_BOX_ITEM_HEIGHT, 1);
      if (nk_button_label(ctx, "Take a picture")) { puts("placeholder"); }
    }
}
...
void GUI_draw_view_panel(struct nk_context *ctx, struct GUI_view_settings *settings) {
  ...
  if (nk_begin(ctx, "Main", nk_rect(0, GUI_TRANSFORM_PANEL_Y_START, GUI_WIN_WIDTH_X, GUI_TRANSFORM_PANEL_Y_END), NK_WINDOW_BORDER | NK_WINDOW_BACKGROUND | NK_WINDOW_NO_SCROLLBAR )) {
    ...
    if (nk_option_label(ctx, "Parallel", (int)settings->projection_type == 0)) settings->projection_type = 0;
    if (nk_option_label(ctx, "Central", (int)settings->projection_type == 1)) settings->projection_type = 1;

    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_LABEL_HEIGHT, 1);
    nk_label(ctx, "Edge type", NK_TEXT_CENTERED);
    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 2);
    if (nk_option_label(ctx, "Parallel", settings->edge_type == 0)) settings->edge_type = 0;
    if (nk_option_label(ctx, "Central", settings->edge_type == 1)) settings->edge_type = 1;

    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    nk_property_float(ctx, "Edge Thickness", 1.0f, &settings->edge_thickness, 10.0f, 0.1f, 0.1f);

    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_LABEL_HEIGHT, 1);
    nk_label(ctx, "Edge Color", NK_TEXT_LEFT);
    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    //struct nk_colorf edge_col = nk_rgb_cf(nk_color_fv(settings->edge_color));  // Convert to nk_colorf
    struct nk_colorf edge_col = {settings->edge_color[0], settings->edge_color[1], settings->edge_color[2], 1.0f};  // Initialize nk_colorf
    ...
    if (nk_option_label(ctx, "None", settings->vertex_display == 0)) settings->vertex_display = 0;
    if (nk_option_label(ctx, "Circle", settings->vertex_display == 1)) settings->vertex_display = 1;
    if (nk_option_label(ctx, "Square", settings->vertex_display == 2)) settings->vertex_display = 2;

    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    nk_property_float(ctx, "Vertex Size", 1.0f, &settings->vertex_size, 10.0f, 0.1f, 0.1f);

    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_LABEL_HEIGHT, 1);
    nk_label(ctx, "Vertex Color", NK_TEXT_LEFT);
    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    struct nk_colorf vertex_col = {settings->vertex_color[0], settings->vertex_color[1], settings->vertex_color[2], 1.0f};  
    ...
    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    struct nk_colorf bg_col = {settings->bg_color[0], settings->bg_color[1], settings->bg_color[2], 1.0f};  
    ...
    nk_layout_row_dynamic(ctx, GUI_VIEW_PANEL_BOX_ITEM_HEIGHT, 1);
    if (nk_button_label(ctx, "Save Settings")) { save_settings(settings); }
...
void GUI_browse_obj_files(struct nk_context *ctx, Model **model, struct GUI_directory_data *data) {
    ...
    if (nk_begin(ctx, "OBJ File Browser", nk_rect(0, GUI_FILE_BROWSE_Y_START, GUI_WIN_WIDTH_X, GUI_FILE_BROWSE_HEIGHT),  // Увеличим высоту окна
        ...
        if (data->file_menu_active && data->obj_file_count > 0) {
            ...
            for (int i = 0; i < data->obj_file_count; i++) {
                ...
                if (nk_button_label(ctx, data->obj_files[i])) {
                    ...
                    int total_length = strlen(data->obj_directory) + strlen(data->obj_files[i]) + 1; // +1 для '/'
                    if (total_length < MAX_PATH_LENGTH) {
                        // Сохраняем выбранный файл
                        snprintf(data->selected_obj_file, MAX_PATH_LENGTH, "%s/%s", data->obj_directory, data->obj_files[i]);
        ...
        int selected_file_length = strlen(data->selected_obj_file);  // Получаем текущую длину строки
        nk_layout_row_dynamic(ctx, 30, 1);
        nk_edit_string(ctx, NK_EDIT_READ_ONLY, data->selected_obj_file, &selected_file_length, MAX_PATH_LENGTH, nk_filter_default);
