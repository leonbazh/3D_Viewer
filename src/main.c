...
void render_model(Model *model, GUI_view_settings *settings, struct GUI_transform_panel *unit) {
    if (model == NULL) return;

    glClearColor(settings->bg_color[0], settings->bg_color[1], settings->bg_color[2], 1.0f);  // Set background color
    ...
    glColor3f(settings->edge_color[0], settings->edge_color[1], settings->edge_color[2]);
    ...
    glTranslatef(unit->translation[0], unit->translation[1], unit->translation[2]);
...
int main(void)
{
...

    {
      ...
      struct nk_font *hero = nk_font_atlas_add_from_file(atlas, "Roboto-Regular.ttf", 16, &cfg);
  ...
  GUI_view_settings settings = {0, 0, {1.0f, 1.0f, 1.0f}, 1.0f, 0, {1.0f, 0.0f, 0.0f}, 5.0f, {0.0f, 0.0f, 0.0f}};
; 
  //load_settings(&settings); // If good - settings from .cfg will be in use
 ...
 while (!glfwWindowShouldClose(win))
{
  ...
  // Если модель не загружена, запускаем процесс выбора файла
  if (model == NULL) {
    ...
    // После выбора файла, проверяем, загружена ли модель
    if (model != NULL) {
    ...
    // Если модель загружена, продолжаем работу
    ...
    // Проверяем модель перед рендерингом
    if (model != NULL) {
