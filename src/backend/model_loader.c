 ...
 *
 * Эта функция загружает каркасную модель из файла формата OBJ. Она парсит
 * вершины и грани из файла и возвращает указатель на структуру Model. Если файл
 * не может быть открыт или данные в файле некорректны, функция возвращает NULL
 * и выводит сообщение об ошибке.
 *
 * @param filepath Путь к OBJ файлу.
 * @return Указатель на структуру Model, содержащую вершины и грани модели, или
 ...
 *
 * Эта функция освобождает память, выделенную под массивы вершин и граней
 * модели, а затем освобождает саму структуру Model.
 *
